# 💾 Intro: Data Transformation with `"dplyr"` 🌻
...
- ฟีเจอร์เหมือนกันเลย แต่การแสดงผล tibble จะอ่านง่ายกว่า สวยงามกว่า data.frame 
...
```
## 🔐 Sample Data
- `sample_n()` สุ่มจำนวน rows ตามที่เราต้องการ
- `sample_frac()`
   - สุ่มเป็น % ได้ i.e. frac ย่อมาจากคำว่า fraction
   - สุ่มจากความน่าจะเป็น หรือ จำนวนเปอร์เซนต์ของข้อมูล
```r
#sample_n

set.seed(42)  ## --> ถ้าอยากล็อคค่า ให้ใส่ set.seed
sample_n(mtcars, size=5) ## --> รันแบบสุ่มตัวอย่าง

sample_frac(mtcars, size=0.5, replace=T)  
## --> replace = TRUE คือให้ผลลัพธ์ที่ดึงมา มีค่าซ้ำกันได้ หรือดึงค่าซ้ำกันขึ้นมาได้
## --> size 0.5 = สุ่มตัวอย่างขึ้นมา 50 % จากข้อมูลทั้งหมด ,size = 0.2 == 20 %
```
## 🔐 Slice
- `slice()` ใช้ดึง rows ที่เราต้องการจาก dataframe / tibble
- slice == คือการใช้งานเพื่อดึงแถวที่เราต้องการออกมา
